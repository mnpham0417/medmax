...
#SBATCH --mem=64GB
#SBATCH --job-name=dog_lora_image_only_no_boi_eoi
#SBATCH --mail-type=ALL
#SBATCH --mail-user=mp5847@nyu.edu
#SBATCH --output=train_dog_lora_image_only_no_boi_eoi_%j.out
...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=29501 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/dog_erasure/metadata_final.jsonl \
                        --ckpt /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/anole_7b_hf \
                        --output_dir /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/dog_lora_image_only_no_boi_eoi \
                        --epoch 50 \
                        --bs 1 \
                        --save_strategy steps \
                        --save_steps 500 \
                        --warmup_ratio 0.1 \
                        --name dog_lora_image_only_no_boi_eoi \
