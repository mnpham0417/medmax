...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/dog_erasure/metadata_final_with_boi_eoi.jsonl \
                        --ckpt /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/anole_7b_hf \
                        --output_dir /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/dog_whole_model_image_only_with_boi_eoi \
                        --epoch 50 \
                        --bs 1 \
                        --save_strategy steps \
                        --save_steps 500 \
                        --warmup_ratio 0.1 \
                        --name dog_whole_model_image_only_with_boi_eoi \
