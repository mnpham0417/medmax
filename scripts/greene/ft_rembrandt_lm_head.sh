...
#SBATCH --mem=64GB
#SBATCH --job-name=rembrandt_lm_head
#SBATCH --mail-type=ALL
#SBATCH --mail-user=mp5847@nyu.edu
#SBATCH --output=train_rembrandt_lm_head_%j.out
...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/rembrandt/metadata_final.jsonl \
                                          ...
                                          --output_dir /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/rembrandt_lm_head \
                                          --epoch 5000 \
                                          --bs 16 \
                                          --save_strategy steps \
                                          --save_steps 5000 \
                                          --warmup_ratio 0.1 \
                                          --name rembrandt_lm_head \
                                          --trainable_layers lm_head \
                                          --lr 1e-3 --grad_acc 1 --wandb --wandb_entity Medmax --name rembrandt_lm_head --wandb_disable_ssl --bf16" 
