...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/dog/metadata_final.jsonl \
                                          ...
                                          --output_dir /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/checkpoints/dog_lm_head \
                                          --epoch 100 \
                                          --bs 1 \
                                          --save_strategy steps \
                                          --save_steps 1000 \
                                          ...
                                          --lr 1e-3 --grad_acc 1 --wandb --wandb_entity Medmax --name dog_lm_head --wandb_disable_ssl --bf16" 
