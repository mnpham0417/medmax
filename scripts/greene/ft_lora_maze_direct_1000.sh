...
#SBATCH --mem=64GB
#SBATCH --job-name=maze_lora_direct_1000
#SBATCH --mail-type=ALL
#SBATCH --mail-user=mp5847@nyu.edu
#SBATCH --output=train_maze_lora_direct_1000_%j.out
...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/maze_dataset/train_metadata_direct_1000.jsonl \
                        --ckpt ./checkpoints/anole_7b_hf \
                        --output_dir ./checkpoints/maze_direct_1000 \
                        --epoch 40 \
                        --bs 1 \
                        --save_strategy steps \
                        --save_steps 5000 \
                        --warmup_ratio 0.1 \
                        --name maze_direct_1000 \
