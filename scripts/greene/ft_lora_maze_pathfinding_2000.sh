...
#SBATCH --mem=64GB
#SBATCH --job-name=maze_lora_pathfinding_2000
#SBATCH --mail-type=ALL
#SBATCH --mail-user=mp5847@nyu.edu
#SBATCH --output=train_maze_lora_pathfinding_2000_%j.out
...
singularity exec --nv \
	...
	/bin/bash -c "source /share/apps/anaconda3/2024.02/etc/profile.d/conda.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/medmax;
					 CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=29501 -m training.train --train_data /scratch/mp5847/workspace/mixed-modal-erasure/src/medmax/data/maze_dataset/train_metadata_pathfinding_2000.jsonl \
                        --ckpt ./checkpoints/anole_7b_hf \
                        --output_dir ./checkpoints/maze_pathfinding_2000 \
                        --epoch 80 \
                        --bs 1 \
                        --save_strategy steps \
                        --save_steps 5000 \
                        --warmup_ratio 0.1 \
                        --name maze_pathfinding_2000 \
