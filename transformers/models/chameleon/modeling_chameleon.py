...
class ChameleonSdpaAttention(ChameleonAttention):
    ...
    def forward(
        ...
        # print(f'value states: {value_states.shape} | position_ids: {position_ids.shape}')
        cos, sin = self.rotary_emb(value_states, position_ids)
...
class ChameleonImageVocabularyMapping:
    ...
    def __init__(self, vocab_map, new_merges = None):
        self.vocab_map = vocab_map
        self.image_token_id = vocab_map.get("<image>")
        self.new_merges = new_merges
        if self.new_merges != None:
            self.merge_map = self.new_merges[0]
            self.imagebpe = self.new_merges[1]
        else:
            self.merge_map = None
            self.imagebpe = None
    ...
    def convert_img2bpe(self, img_batch: torch.Tensor) -> torch.Tensor:
        ...
        # this is where we will tokenize it again based on our new merges
        if self.imagebpe != None:
            img_tokens = img_tokens.view(-1, 1024)
            img_tokens = img_tokens.tolist()
            new_img_tokens = []
            for img_tokens_list in img_tokens:
                img_tokens_list = self.imagebpe(img_tokens_list, self.merge_map)
                img_tokens_list = torch.tensor(img_tokens_list).to(device)
                new_img_tokens.append(img_tokens_list)
            img_tokens = new_img_tokens
        else:
